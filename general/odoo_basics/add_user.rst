...
| Fill in the form with the needed information. Under the tab
  ...
  each application the user can have access.
...
the user. The user must click on it to accept the invitation to your instance and create a login.
...
.. note::
   ...
   `pricing page <https://www.odoo.com/pricing#pl=66&version_id=3&num_users=1&hosting=online&odoosh_workers=1&odoosh_storage=1&odoosh_staging=1&implementation_service=self&pack=25&force_country=BE&integrating_partner_id=0&price_by=yearly>`_
...
ones (such as record rules and restricted menus), and they usually do not belong to the usual Odoo
groups.
...
The *Menus* tab is where you define which menus (models) within the application the user can have
access to.
...
- *Write*: the values of that object can be edit by the user.
...
| A record rule is written using a *Domain*. Domains are conditions used to filter or searching
  data. Therefore, a domain expression is a list of conditions. For each rule here created, choose
  among the following options: *Read*, *Write*, *Create* and *Delete* values.
